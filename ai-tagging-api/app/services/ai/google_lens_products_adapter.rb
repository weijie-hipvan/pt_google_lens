  ...
  class GoogleLensProductsAdapter
    ...
    def self.search(image_data:, options: {})
      ...
      params = {
        ...
        gl: options[:country] || 'sg', # Default to Singapore
      ...
      handle_serpapi_response(response, processing_time_ms)
    ...
    def self.handle_serpapi_response(response, processing_time_ms)
      if response.success?
        ...
        products = (body['visual_matches'] || []).map do |item|
          ...
          
          {
            ...
            currency: item.dig('price', 'currency') || detect_currency(raw_price),
